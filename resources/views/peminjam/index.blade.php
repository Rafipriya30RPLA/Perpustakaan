  ...
  <div class="row">
    <div class="col-md-12 grid-margin stretch-card">
      <div class="card">
        ...
        <div class="card-body">
          <p class="card-title">Daftar Peminjam</p>
          <div class="row">
                        ...
                        <td>
                          <div class="d-flex">
                            <form action="{{ route('peminjam.update', $row->id) }}"method="POST"
                              ...
                              @csrf
                              @method('POST')
                              <button type="submit" class="btn btn-outline-success mr-1">Setujui</button>
                            </form>
                            <form action="{{ route('peminjam.destroy', $row->id) }}" method="post">
                              @method('DELETE')
                              @csrf
                              <button type="button" class="btn btn-outline-danger delete-btn"
                                style="margin-left: 10px;" data-id="{{ $row->id }}">Hapus</button>
                            </form>
                          </div>
                        </td>
                      </tr>
